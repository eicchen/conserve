...
async def lifespan(app: FastAPI):
    ...
    for i, (host, port) in enumerate(decoder_pairs):
        ...
        print(f"Added decoder client: {decoder_base_url}, {host}-{init_ports}-{alloc_ports}", flush=True)
...
async def send_request_to_service(
    ...
    try:
        ...
        print(f"send_request_to_service: Exception occurred: {type(e).__name__}: {e}", flush=True)
        raise
...
def pick_decoder_by_index(decoder_index: int) -> tuple[ClientInfo, ClientInfo, ClientInfo]:
    """Pick a specific decoder by index. Tokenization and prefill fall back to index 0."""
    tokenization_client = app.state.total_clients[0]
    prefill_client = app.state.prefill_clients[0]
    decode_client = app.state.decode_clients[decoder_index % len(app.state.decode_clients)]
    return tokenization_client, prefill_client, decode_client


def pick_up_clients(request: Request, req_data: dict) -> tuple[ClientInfo, ClientInfo, ClientInfo]:
    decoder_index = req_data.pop("decoder_index", None)
    if decoder_index is not None:
        return pick_decoder_by_index(int(decoder_index))
...
async def handle_completions(request: Request):
    ...
    try:
        ...
        tokenization_client, prefill_client, decode_client = pick_up_clients(request, req_data)
        ...
        #     print(f"decoder ready for req {req_id}")
        ...
        decode_output = await service_response(
            decode_client.client, "/v1/completions", req_data
        )

        final_text = (
            prefill_output["choices"][0]["text"]
            + decode_output["choices"][0]["text"]
        )
            ...
            "choices": [
                {
                    ...
                    "text": final_text,
                    "logprobs": None,
                    "finish_reason": decode_output["choices"][0]["finish_reason"],
                    "stop_reason": decode_output["choices"][0].get("stop_reason"),
                }
            ],
            "usage": decode_output.get("usage"),
        }
