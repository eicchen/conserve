"""Standalone copy of conserve/input/build_agent_trace.py for the fig1
sbatch pipeline (see run_fig1_trace.sbatch in this directory).

Differs from the canonical conserve/input/build_agent_trace.py only in that
it imports the local config.py copy in this directory (which honors
CONSERVE_CONFIG_ENV) instead of config/config.py, so each sbatch job's
isolated BENCHMARK override is picked up correctly.

The canonical conserve/input/build_agent_trace.py is untouched; this copy is
...
Then optionally plots Figure 1.

Usage
-----
    python build_agent_trace.py            # build trace + plot
    python build_agent_trace.py --no-plot  # trace only
"""
